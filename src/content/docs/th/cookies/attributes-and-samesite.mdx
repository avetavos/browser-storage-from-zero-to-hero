...
**ไม่มี Expires/Max-Age** → session cookie browser ลบมันเมื่อ session แท็บ/หน้าต่างสิ้นสุด (พฤติกรรมการปิด browser แตกต่างกันตาม browser และการตั้งค่า "restore session")
...
browser จะส่ง cookie นี้เฉพาะ request ที่ path ขึ้นต้นด้วย `/admin` เท่านั้น Request ไปยัง `/` หรือ `/app` จะไม่รวมมัน
...
ถ้าไม่ระบุ `Domain` cookie จะถูกส่งเฉพาะ host ที่ตั้งค่ามันเท่านั้น (ไม่มี subdomain)
...
`HttpOnly` คือหนึ่งใน attribute ด้านความปลอดภัยที่สำคัญที่สุด มันป้องกันไม่ให้ JavaScript อ่าน cookie ผ่าน `document.cookie`:
...
<Callout title="cookie HttpOnly มองไม่เห็นจาก JavaScript — นี่คือฟีเจอร์ ไม่ใช่บัก">
`HttpOnly` หมายความว่า `document.cookie` จะไม่เคยคืน cookie นั้น และ script ไม่สามารถอ่านหรือแก้ไขได้ นี่เป็นสิ่งที่ตั้งใจ: ช่วยปกป้อง session token จากการถูกขโมยโดย script ที่ถูกฉีดเข้ามา ถ้าคุณมองไม่เห็น cookie ใน `document.cookie` ที่คุณรู้ว่า server ตั้งค่า นั่นเกือบจะแน่นอนว่ามัน `HttpOnly` — ตรงตามที่ออกแบบไว้
</Callout>
