...
- สำหรับแอปที่ต้องทำงานออฟไลน์ ควรเก็บข้อมูล diagnostic ไว้ใน localStorage — เพราะมี quota คงที่ขนาดเล็กของตัวเองและไม่อยู่ภายใต้ระบบ eviction จาก storage pressure ที่ควบคุม IndexedDB, Cache API และ OPFS — เพื่อตรวจจับการสูญเสียข้อมูลเมื่อโหลดหน้าครั้งถัดไป
...
**Twitter Lite (PWA)** — ขอ persistent storage เพื่อป้องกันไม่ให้ app shell ที่ cache ไว้ถูก evict ระหว่างที่ผู้ใช้ engagement ต่ำ แหล่งอ้างอิง: [web.dev case study, "Twitter Lite PWA significantly increases engagement and reduces data usage"](https://web.dev/case-studies/twitter)
</Callout>
...
export const quizEvictionAndPersistence = [
  {
    ...
    options: [
      ...
      'Best-effort — browser อาจ evict ข้อมูลภายใต้ storage pressure',
  ...
  {
    q: 'เมื่อ browser evict origin แบบ best-effort เนื่องจาก storage pressure อะไรถูกล้าง?',
    options: [
  ...
  {
    q: 'navigator.storage.persist() คืนค่าอะไรถ้า browser ปฏิเสธคำขอ?',
    options: [
      'throw DOMException',
      'false',
      'undefined',
