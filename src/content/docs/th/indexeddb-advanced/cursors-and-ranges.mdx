...
Cursor ไม่ได้โหลด record ทั้งหมดเข้าหน่วยความจำพร้อมกัน — แต่เดินผ่านทีละรายการ ต้องเรียก `cursor.continue()` ภายใน handler ของ `onsuccess` เสมอเพื่อเลื่อนไปหน้า หากลืมเรียกจะทำให้การ iterate หยุดนิ่งโดยไม่มีข้อผิดพลาด จับคู่ cursor กับ `IDBKeyRange` เพื่อหลีกเลี่ยงการสแกน record ที่ไม่จำเป็น
</Callout>
...
export const quizCursorsAndRanges = [
  {
    ...
    options: ['undefined', 'null', '0', 'จะโยน StopIteration error'],
    answer: 1,
  },
  {
