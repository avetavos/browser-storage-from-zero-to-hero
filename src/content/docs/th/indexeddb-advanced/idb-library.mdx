...
`idb` ห่อหุ้มทุก operation IndexedDB ด้วย Promise ทำให้คุณสามารถใช้ `async/await` แทน event callback ที่ซ้อนกัน Callback `upgrade` ภายใน `openDB` เป็นสถานที่เดียวในการสร้างหรือแก้ไข object store — จะรันเฉพาะเมื่อหมายเลขเวอร์ชันเพิ่มขึ้น
</Callout>
...
export const quizIdbLibrary = [
  ...
  },
  {
    ...
    options: [
      'จัดเก็บข้อมูลในรูปแบบที่เร็วกว่า IndexedDB ดิบ',
      'อ่านและเขียนข้อมูลแบบ synchronous โดยไม่ต้องใช้โค้ด async',
      'ห่อหุ้มทุกการดำเนินการด้วย Promise ทำให้ใช้ async/await แทน nested callback ได้',
      'เพิ่มโควต้าการจัดเก็บข้อมูลที่มีให้กับ origin',
    ],
    answer: 2,
  },
];
