...
ใช้ `add` เมื่อต้องการความแน่ใจว่ากำลังสร้าง record ใหม่; จะโยน error เมื่อ key ซ้ำ ใช้ `put` เมื่อต้องการพฤติกรรม upsert และไม่สนใจว่า record มีอยู่แล้วหรือไม่
...
export const quizCrud = [
  {
    q: 'คุณเรียก store.add({ id: 1, name: "Apple" }) ตอนที่ record ที่มี id 1 มีอยู่แล้ว จะเกิดอะไรขึ้น?',
    options: [
  ...
  },
  {
    ...
    options: ['null', 'false', 'undefined', 'โยน NotFoundError'],
    answer: 2,
  },
  {
